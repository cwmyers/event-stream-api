...
case class GetLatestSnapshot(entityId: EntityId, systemName: SystemName, time: OffsetDateTime)
    extends AppAction[Option[Snapshot]]
    with EventStoreAction[Option[Snapshot]]
...
object EventStoreAction {
  ...
  def listEventsByRange(id: EntityId,
                        systemName: SystemName,
                        from: MaybeTime,
                        to: OffsetDateTime): Script[List[Event]] =
